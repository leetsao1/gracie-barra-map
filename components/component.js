...
const Component = () => {
    ...
    (location) => {
      ...
      <div class="${styles.locationCard}" data-location-id="${locationId}">
        <div class="${styles.locationHeader}" data-draggable="true">
          ...
          }" data-popup-close="true" aria-label="Close popup">×</button>
    ...
    (popup, coordinates, locationId, locationData) => {
      ...
      try {
        ...
        const newPopup = new mapboxgl.Popup({
          ...
          maxWidth: isMobile ? "95vw" : "600px", // Responsive width
          maxHeight: isMobile ? "85vh" : "none", // Limit height on mobile
          closeOnClick: false,
          anchor: isMobile ? "bottom" : "bottom", // Better positioning on mobile
          offset: isMobile ? [0, -10] : [0, 0], // Slight offset on mobile for better visibility
          className: isMobile ? "mobile-popup" : "", // Add mobile class for styling
        });
        ...
        // Add event listeners and drag functionality after popup is rendered
        setTimeout(() => {
          try {
            ...
            // Add close button event listener (mobile-safe)
            ...
            if (closeButton) {
              const handleClose = (e) => {
                ...
                try {
                  ...
                  // Fallback: try to remove the popup element directly
                  try {
                    ...
                    console.error("Fallback close failed:", fallbackErr);
                  }
                }
              };
              ...
              newPopup._closeHandler = handleClose;
            }

            // Add drag functionality (disabled on mobile)
            if (!isMobileDevice()) {
              makePopupDraggable(popupContent, newPopup);
            }
          } catch (error) {
            console.error("Error setting up popup:", error);
            // Don't crash - popup will still work, just without close button handler
          }
        }, 100);

        // Set up close handler with proper cleanup
        newPopup.on("close", () => {
          try {
            // Clean up close button event listeners
            if (newPopup._closeHandler) {
              const popupElement = newPopup.getElement();
              if (popupElement) {
                const popupContent = popupElement.querySelector(
                  ".mapboxgl-popup-content"
              ...
              delete newPopup._closeHandler;
            }
          } catch (err) {
            console.error("Error cleaning up popup:", err);
          }
  ...
  const filterAndDisplayLocations = useCallback(
    async (locations = allLocations) => {
      ...
      const BATCH_SIZE = 100; // Create 100 markers at a time
      let currentBatch = 0;
        ...
        for (let i = start; i < end; i++) {
          ...
          if (location.coordinates && location.coordinates.length === 2) {
            ...
            const handleMarkerClick = (e) => {
              ...
              // Create and open popup with mobile-optimized settings
              const isMobile = isMobileDevice();
              ...
              if (content) {
                ...
                setTimeout(() => {
                  try {
                    ...
                    if (closeButton) {
                      ...
                      // Store handler for cleanup
                      popup._closeHandler = handleClose;
                    }
                ...
                // Set up close handler and store popup reference with proper cleanup
                popup.on("close", () => {
                  try {
                    // Clean up close button event listeners
      ...
      if (typeof addressOrCoords === "string") {
        ...
        try {
          ...
          if (directMatch && directMatch.coordinates) {
            ...
            // If no direct match, use server-side geocoding API
            const response = await fetch('/api/geocode', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
    ...
    (location) => {
      try {
        ...
        if (!location || !location.coordinates) {
          ...
          if (isMobileDevice()) {
            setLocationError("Invalid location data. Please try selecting another location.");
          }
          return;
        }
        ...
        if (mapInstance.current) {
          ...
          // Longer delay on mobile to ensure map animation completes
          ...
          setTimeout(() => {
            ...
            try {
              ...
              if (isMobileDevice()) {
                setLocationError("Unable to display location details. Please try again.");
              }
            }
          }, delay);
        } else {
          ...
          if (isMobileDevice()) {
            setLocationError("Map is not ready. Please wait a moment and try again.");
          }
        }
        ...
        // Prevent crash on mobile - show user-friendly error
        if (isMobileDevice()) {
          setLocationError("An error occurred. Please try selecting the location again.");
        }
      }
    },
    [
  ...
  const handleResultItemClick = useCallback(
    (location, event) => {
      try {
        ...
        // Add small delay on mobile to ensure touch event is fully processed
        if (isMobileDevice() && event && event.type === 'touchend') {
          setTimeout(() => {
        ...
        // Prevent crash on mobile - show user-friendly error
        if (isMobileDevice()) {
          setLocationError("Unable to open location details. Please try again.");
        }
      }
    },
              ...
              searchResults.map((result) => (
                <div
                  ...
                  onTouchEnd={(e) => {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    handleResultItemClick(result, e);
                  }}
